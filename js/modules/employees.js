...
function renderEmployeesTable() {
  ...
  $("employeesTable").innerHTML = `
    <div class="grid cards" style="grid-template-columns:repeat(4,minmax(0,1fr));margin-bottom:14px">
      ...
      ${canSeeFinancials() ? `<div class="card"><div class="stat-title">Average Rate</div><div><b>${money(avgRate())}</b></div></div>` : ""}
      <div class="card"><div class="stat-title">Branches</div><div><b>${new Set(employees.map(e => e.branch_id)).size}</b></div></div>
    </div>
    <table>
      <thead><tr><th>Employee</th><th>Branch</th><th>Role</th><th>Employment</th>${canSeeFinancials() ? "<th>Rate</th>" : ""}<th>Status</th><th></th></tr></thead>
      <tbody>
        ${rows.map(e => `<tr>
          ...
          ${canSeeFinancials() ? `<td>${money(e.hourly_rate)}</td>` : ""}
...
function openEmployeeModal(employee = null) {
    ...
    <form id="employeeForm">
      <div class="modal-body">
        <div class="form-grid">
          ...
          ${canSeeFinancials() ? `<div><label>Hourly Rate</label><input name="hourly_rate" type="number" step="0.01" class="input" value="${esc(employee?.hourly_rate ?? 0)}"></div>` : ""}
  ...
  $("employeeForm").onsubmit = async e => {
    ...
    const payload = {
      ...
      hourly_rate: canSeeFinancials() ? Number(fd.get("hourly_rate") || 0) : Number(employee?.hourly_rate || 0),
