...
export async function renderReceiving() {
  ...
  try {
    ...
    content.innerHTML = `
      <div class="card">
        ...
        <div class="muted" style="margin-bottom:12px">
          This screen shows approved and partially received purchase orders for the selected branch.
        </div>
    ...
    $("recSupplier").onchange = e => {
      filters.supplier_id = e.target.value;
      renderTables();
    };
  ...
  $("receivingTable").innerHTML = `
    <table>
      ...
      <tbody>
        ...
        `).join("") || '<tr><td colspan="7" class="muted">No approved purchase orders ready for receiving.</td></tr>'}
      </tbody>
    </table>
  `;
        ...
        ${recent.map(rn => {
          ...
          return `
            <tr>
              ...
              <td><button class="btn secondary small copy-rn" data-id="${esc(rn.id)}">Copy Text</button></td>
            </tr>
          `;
...
function receivedBefore(poId, poLineId) {
  const relatedNoteIds = new Set(
    receivingNotes
      .filter(n => (n.po_id || n.purchase_order_id) === poId)
      .map(n => n.id)
  );

  return receivingLines
    .filter(l =>
      relatedNoteIds.has(l.grn_id || l.receiving_note_id) &&
      (l.po_line_id || l.purchase_order_line_id) === poLineId
    )
...
async function openReceivingModal(po) {
      ...
      <div class="modal-body">
        <div class="form-grid">
          <div>
            ...
            <input class="input" value="${esc(supplierName(supplier(po.supplier_id)))}" disabled>
          </div>

          <div>
            <label>Receiving Date</label>
            <input name="received_date" type="date" class="input" value="${new Date().toISOString().slice(0,10)}">
          </div>
        ...
        <div style="text-align:right;font-weight:900;font-size:20px;margin-top:14px">
          Receiving Total: <span id="receivingTotal">$0.00</span>
        </div>
      </div>
      ...
      <table>
        ...
        <tbody>
          ${local.map((line, index) => `
            <tr>
              <td><b>${esc(line.item_name)}</b><div class="muted">${esc(line.unit)} / cost ${esc(line.cost_unit)}</div></td>
              <td>${qty(line.ordered_qty)} ${esc(line.unit)}</td>
              <td>${qty(line.received_before_qty)} ${esc(line.unit)}</td>
              <td><input type="number" step="0.001" class="input delivered-input" data-index="${index}" value="${esc(line.delivered_qty)}"></td>
              ...
              <td>
                ${sameUnit(line.unit, line.cost_unit)
                  ? '<span class="muted">Auto</span>'
                  : `<input type="number" step="0.001" class="input costqty-input" data-index="${index}" value="${esc(line.actual_cost_qty ?? "")}" placeholder="${esc(line.cost_unit)}">`}
              </td>
  ...
  function bindLineInputs() {
    ...
    document.querySelectorAll(".accepted-input").forEach(input => {
      input.oninput = () => {
        const line = local[Number(input.dataset.index)];
        line.accepted_qty = Number(input.value || 0);
        drawLines();
      };
    });
    ...
    document.querySelectorAll(".costqty-input").forEach(input => {
      input.oninput = () => {
        const line = local[Number(input.dataset.index)];
        line.actual_cost_qty = Number(input.value || 0);
        drawLines();
      };
    });
  ...
  $("receivingForm").onsubmit = async event => {
    ...
    const activeLines = local.filter(line =>
    ...
    if (!activeLines.length) {
      toast("Enter at least one delivered quantity.", "error");
      return;
    }
    ...
    try {
      ...
      for (const line of activeLines) {
        if (Number(line.accepted_qty || 0) > 0) {
          await addStockMovement(note, po, line);
        }
      }
...
async function updatePoStatus(poId) {
  const poLines = await safeSelect("purchase_order_lines", "*", {
    eq: { purchase_order_id: poId }
  }).catch(() => []);

  const notesForPo = await safeSelect("receiving_notes", "*", {
    eq: { po_id: poId }
  }).catch(() => []);
  ...
  for (const poLine of poLines) {
    ...
    const received = relatedLines
      .filter(line => (line.po_line_id || line.purchase_order_line_id) === poLine.id)
      .reduce((sum, line) => sum + Number(line.accepted_qty || 0), 0);
