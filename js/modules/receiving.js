...
let noteFilters = { supplier_id:"", from:"", to:"", search:"" };
...
function lineTotal(line){ return sameUnit(line.unit,line.cost_unit) ? Number(line.accepted_qty||0)*Number(line.unit_price||0) : Number(line.actual_cost_qty||0)*Number(line.unit_price||0); }
...
export async function renderReceiving(){
  ...
  try{
    ...
    $("rnSupplier").value=noteFilters.supplier_id; $("rnFrom").value=noteFilters.from; $("rnTo").value=noteFilters.to; $("rnSearch").value=noteFilters.search;
...
function filteredNotes(){
  ...
  return receivingNotes.filter(n=>{
    ...
    const text=`${rnNo(n)} ${poNo(po)} ${JSON.stringify(n)}`.toLowerCase();
...
function renderRecentNotes(){
  ...
  $("rnSummary").innerHTML=`Branch: <b>${esc(branchName())}</b> | Notes: <b>${rows.length}</b> | Total payable/received: <b>${money(total)}</b>`;
  $("recentReceiving").innerHTML=`<table><thead><tr><th>RN</th><th>PO</th><th>Branch</th><th>Supplier</th><th>Date</th><th>Total</th><th></th></tr></thead><tbody>
  ${rows.map(rn=>{const po=notePo(rn);return `<tr><td><b>${esc(rnNo(rn))}</b></td><td>${esc(poNo(po))}</td><td>${esc(branchName())}</td><td>${esc(supplierName(supplier(rn.supplier_id||po.supplier_id)))}</td><td>${esc((rn.received_date||rn.received_at||rn.created_at||"").slice(0,10))}</td><td>${money(rn.total_amount)}</td><td><button class="btn secondary small open-rn" data-id="${esc(rn.id)}">Open</button><button class="btn secondary small copy-rn" data-id="${esc(rn.id)}">Copy</button><button class="btn secondary small pdf-rn" data-id="${esc(rn.id)}">PDF</button><button class="btn secondary small email-rn" data-id="${esc(rn.id)}">Email</button></td></tr>`;}).join("")||'<tr><td colspan="7" class="muted">No receiving notes found.</td></tr>'}</tbody></table>`;
...
async function openReceivingModal(po){
  ...
  const local=poLines.map(line=>{const it=item(line.item_id),unit=line.order_unit||line.unit||it?.receiving_unit||it?.stock_unit||"",costUnit=line.cost_unit||it?.cost_unit||it?.stock_unit||unit,already=receivedBefore(po.id,line.id),ordered=Number(line.ordered_qty||0);return{po_line_id:line.id,item_id:line.item_id,item_name:itemLabel(it),ordered_qty:ordered,received_before_qty:already,remaining_qty:Math.max(0,ordered-already),delivered_qty:0,accepted_qty:0,rejected_qty:0,unit,stock_unit:it?.stock_unit||unit,cost_unit:costUnit,actual_cost_qty:sameUnit(unit,costUnit)?null:0,unit_price:Number(line.unit_price||0),reject_reason:"",notes:""};});
  ...
  function drawLines(){const anyCost=local.some(l=>!sameUnit(l.unit,l.cost_unit));$("receivingLinesBox").innerHTML=`<table><thead><tr><th>Item</th><th>Ordered</th><th>Received Before</th><th>Delivered Now</th><th>Accepted</th><th>Rejected</th>${anyCost?"<th>Billing Qty</th>":""}<th>Total</th><th>Reject Reason</th></tr></thead><tbody>${local.map((line,index)=>{const needs=!sameUnit(line.unit,line.cost_unit);return `<tr data-row="${index}"><td><b>${esc(line.item_name)}</b><div class="muted">Receive in ${esc(line.unit)}${needs?` / bill by ${esc(line.cost_unit)}`:""}</div></td><td>${qty(line.ordered_qty)} ${esc(line.unit)}</td><td>${qty(line.received_before_qty)} ${esc(line.unit)}</td><td><input type="number" step="0.001" class="input delivered-input" data-index="${index}" value="${esc(line.delivered_qty)}" placeholder="${esc(line.unit)}"></td><td><input type="number" step="0.001" class="input accepted-input" data-index="${index}" value="${esc(line.accepted_qty)}" placeholder="${esc(line.unit)}"></td><td><input type="number" step="0.001" class="input rejected-input" data-index="${index}" value="${esc(line.rejected_qty)}" placeholder="${esc(line.unit)}"></td>${anyCost?`<td>${needs?`<input type="number" step="0.001" class="input costqty-input" data-index="${index}" value="${esc(line.actual_cost_qty??"")}" placeholder="Enter ${esc(line.cost_unit)}">`:'<span class="muted">Auto</span>'}</td>`:""}<td class="line-total">${money(sameUnit(line.unit,line.cost_unit)?Number(line.accepted_qty||0)*Number(line.unit_price||0):Number(line.actual_cost_qty||0)*Number(line.unit_price||0))}</td><td><input class="input reason-input" data-index="${index}" value="${esc(line.reject_reason||"")}" placeholder="Reason"></td></tr>`;}).join("")}</tbody></table>`;$("receivingTotal").textContent=money(calculateTotal());bindLineInputs();}
