...
const purchasableItems = selectedId => state.items.filter(i => (i.item_type || "raw") === "raw" || i.id === selectedId);
...
function poPhone(){ return "0404 722 009"; }
...
function renderPoTable(){
  ...
  $("poTable").innerHTML = `<table><thead><tr><th>PO</th><th>Branch</th><th>PO Date</th><th>Supplier</th><th>Status</th><th>PO Total</th><th>Actual Received</th><th></th></tr></thead>
  <tbody>${rows.map(po=>`
    <tr>
...
function receivedValueForLine(poId,lineId){ const ids=relatedNoteIds(poId); return receivingLines.filter(l=>ids.has(l.grn_id||l.receiving_note_id)&&(l.po_line_id||l.purchase_order_line_id)===lineId).reduce((a,l)=>a+receivedLineValue(l),0); }
...
async function openPoModal(po=null){
  ...
  <form id="poForm"><div class="modal-body">
    <div class="form-grid">
      ...
      <div><label>Supplier</label><select name="supplier_id" ${locked?"disabled":""} required>${state.suppliers.map(s=>`<option value="${esc(s.id)}" ${s.id===po?.supplier_id?"selected":""}>${esc(supplierName(s))}</option>`).join("")}</select>${help("Supplier receiving this purchase order.")}</div>
  ...
  function renderLines(){
    ...
    ${localLines.map((l,idx)=>{
      ...
      return `<tr>
      <td><select style="min-width:260px;width:100%" data-idx="${idx}" class="po-item" ${locked?"disabled":""}><option value="">-- Select --</option>${purchasableItems(l.item_id).map(i=>`<option value="${esc(i.id)}" ${i.id===l.item_id?"selected":""}>${esc(itemLabel(i))}</option>`).join("")}</select></td>
  ...
  function updatePoTotalsOnly(){
    const total = localLines.reduce((sum,l)=>sum+lineTotal({...l, unit:l.unit||l.order_unit}),0);
    if($("poTotal")) $("poTotal").textContent = money(total);
    document.querySelectorAll(".po-line-total").forEach((cell, idx)=>{ const l = localLines[idx]; if(l) cell.textContent = lineTotalText({...l, unit:l.unit||l.order_unit}); });
  }
  function bindLineEvents(){
    ...
    document.querySelectorAll(".po-qty").forEach(el=>el.oninput=e=>{ localLines[Number(e.target.dataset.idx)].ordered_qty=Number(e.target.value||0); updatePoTotalsOnly(); });
    document.querySelectorAll(".po-price").forEach(el=>el.oninput=e=>{ localLines[Number(e.target.dataset.idx)].unit_price=Number(e.target.value||0); updatePoTotalsOnly(); });
  ...
  if(!locked){
    document.querySelectorAll("#poForm button[data-save-mode]").forEach(btn=>{ btn.onclick = () => { saveMode = btn.dataset.saveMode || "draft"; }; });
  }
  if(isEdit){
    ...
    document.querySelectorAll(".view-rn-from-po").forEach(btn=>btn.onclick = () => openReceivingNoteFromPo(receivingNotes.find(n => n.id === btn.dataset.id)));
  }
  ...
  $("poForm").onsubmit = async(e)=>{
    ...
    const total=localLines.reduce((sum,l)=>sum+lineTotal({...l,unit:l.unit||l.order_unit}),0);
    const payload={branch_id:state.currentBranchId,supplier_id:fd.get("supplier_id"),status:saveMode==="approve"?"approved":(po?.status||"draft"),order_date:fd.get("order_date")||new Date().toISOString().slice(0,10),expected_delivery_date:fd.get("expected_delivery_date")||null,delivery_asap:fd.get("delivery_asap")==="on",notes:fd.get("notes")||null,total_amount:total,updated_at:new Date().toISOString()};
    if(saveMode === "approve"){ payload.approved_at = new Date().toISOString(); payload.approved_by = state.user.id; }
    try{
      let saved;
      ...
      const clean=localLines.filter(l=>l.item_id).map((l,idx)=>{ const it=getItem(l.item_id), orderUnit=l.unit||l.order_unit||it?.receiving_unit||it?.stock_unit, costUnit=l.cost_unit||it?.cost_unit||it?.stock_unit; return {purchase_order_id:saved.id,item_id:l.item_id,ordered_qty:Number(l.ordered_qty||0),unit:orderUnit,order_unit:orderUnit,cost_unit:costUnit,unit_price:Number(l.unit_price||0),notes:l.notes||null,sort_order:idx}; });
      if(clean.length){const {error}=await state.db.from("purchase_order_lines").insert(clean); if(error)throw error;}
      toast(saveMode === "approve" ? "Purchase order saved and approved." : "Purchase order saved as draft.","ok");
      closeModal(); renderPurchaseOrders();
    }catch(err){toast("PO save failed: "+err.message,"error");}
  };
}

function summaryBlock(po){ const actual=receivedTotalForPo(po.id); return `<div class="grid cards" style="grid-template-columns:repeat(4,minmax(0,1fr));margin-top:16px"><div class="card"><div class="stat-title">Branch</div><div><b>${esc(branchName())}</b></div></div><div class="card"><div class="stat-title">PO Total</div><div><b>${money(po.total_amount)}</b></div><div class="muted">May be 0/pending for variable billing items.</div></div><div class="card"><div class="stat-title">Actual Received Total</div><div><b>${money(actual)}</b></div><div class="muted">Use this for supplier payment.</div></div><div class="card"><div class="stat-title">Outstanding</div><div><b>${money(Math.max(0, Number(po.total_amount||0)-actual))}</b></div></div></div>`; }

function receivingNotesBlock(po){ 
  const notes=relatedNotes(po.id); 
  return `<div class="card" style="margin-top:16px"><div class="section-head"><h3 style="margin:0">Receiving Notes</h3></div><table><thead><tr><th>RN</th><th>Branch</th><th>Date</th><th>Total</th><th>Payment</th><th>View</th></tr></thead><tbody>${notes.map(n=>`<tr><td>${esc(rnNo(n))}</td><td>${esc(branchName())}</td><td>${esc((n.received_date||n.received_at||n.created_at||"").slice(0,10))}</td><td>${money(n.total_amount)}</td><td>${esc(n.payment_status||"unpaid")} (${money(n.paid_amount||0)})</td><td><button type="button" class="btn secondary small view-rn-from-po" data-id="${esc(n.id)}">View</button></td></tr>`).join("") || `<tr><td colspan="6" class="muted">No receiving notes yet.</td></tr>`}</tbody></table></div>`; 
}
function openReceivingNoteFromPo(note){
  if(!note) return;
  const lines = receivingLines.filter(l => (l.grn_id || l.receiving_note_id) === note.id);
  openModal(`<div class="modal-head"><h3>Receiving Note ${esc(rnNo(note))}</h3><button class="btn secondary small" onclick="closeModal()">✕</button></div><div class="modal-body"><table><thead><tr><th>Item</th><th>Delivered</th><th>Accepted</th><th>Rejected</th><th>Billing</th><th>Total</th><th>Reason</th></tr></thead><tbody>${lines.map(l=>`<tr><td>${esc(itemLabel(getItem(l.item_id)))}</td><td>${qty(l.delivered_qty || l.received_qty)} ${esc(l.receive_unit || l.order_unit || "")}</td><td>${qty(l.accepted_qty)}</td><td>${qty(l.rejected_qty || 0)}</td><td>${l.secondary_qty ? `${qty(l.secondary_qty)} ${esc(l.secondary_unit || l.cost_unit || "")}` : (l.cost_qty ? `${qty(l.cost_qty)} ${esc(l.cost_unit || "")}` : "Auto")}</td><td>${money(receivedLineValue(l))}</td><td>${esc(l.rejection_reason || l.reject_reason || "")}</td></tr>`).join("")}</tbody></table></div><div class="modal-foot"><button class="btn secondary" onclick="closeModal()">Close</button></div>`);
}
